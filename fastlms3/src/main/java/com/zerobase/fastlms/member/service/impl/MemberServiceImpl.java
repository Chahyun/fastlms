...
import com.zerobase.fastlms.servlet.CustomRequest;
...
public class MemberServiceImpl implements MemberService {
    ...
    @Override
    public List<LoginHistoryDto> userLoginHistory(String userId) {
        List<LoginHistory> loginHistories = loginHistoryRepository.findAllByUserIdOrderByNumberDesc(userId);
        System.out.println(loginHistories.toString());
        if (CollectionUtils.isEmpty(loginHistories)){
            return null;
        }
        return loginHistories.stream()
                .map(e -> new LoginHistoryDto(
                        e.getNumber(),
                        e.getLoginDt(),
                        e.getIp(),
                        e.getUserAgent()
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        member.setLastLogin(loginTime);
        setLoginHistory(username, loginTime);
        memberRepository.save(member);

        return new User(member.getUserId(), member.getPassword(), grantedAuthorities);
    }

    private void setLoginHistory(String username, String loginTime) {
        Optional<LoginHistory> optionalHistory = loginHistoryRepository.findFirstByUserIdOrderByNumberDesc(username);
        Long number = 1L;
        if(optionalHistory.isPresent()) {
            number = optionalHistory.get().getNumber()+1L;
        }
